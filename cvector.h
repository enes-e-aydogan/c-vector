  ...
  static inline int NAME##_vec_pop(NAME##_vec_t* vec, TYPE* out) {                              \
    ...
    *out = (vec->data[vec->len-- - 1]);                                                         \
    ...
    return 0;                                                                                   \
                                                                                                ...
                                                                                                \
  static inline int NAME##_vec_del(NAME##_vec_t* vec, size_t index, TYPE* out) {                \
    if (!vec || !out || index >= vec->len)                                                      \
      return -1;                                                                                \
    *out = (vec->data[index]);                                                                  \
    if (index < vec->len - 1) {                                                                 \
      memmove(&vec->data[index], &vec->data[index + 1], (vec->len - index - 1) * sizeof(TYPE)); \
    }                                                                                           \
    vec->len--;                                                                                 \
    return 0;                                                                                   \
  }                                                                                             \
...
#define DEFINE_STR_VECTOR                                                            \
  ...
  static inline char* str_vec_pop(str_vec_t* vec) {                                  \
    ...
    return vec->str[vec->len-- - 1];                                                 \
                                                                                     ...
                                                                                     \
  static inline char* str_vec_del(str_vec_t* vec, size_t index) {                    \
    if (!vec || index >= vec->len)                                                   \
      return NULL;                                                                   \
