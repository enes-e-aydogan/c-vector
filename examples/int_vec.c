...
int main(void) {
    ...
    for (size_t i = 0; i < len; i++) {
        int value;
        if (int_vec_get(int_vec, i, &value) == 0) {
            printf("%d ", value);
        }
    }
    printf("\n");
    ...
    // Delete element at TEST_INDEX using _del
    int removed;
    if (int_vec_del(int_vec, TEST_INDEX, &removed) == 0) {
        printf("Deleted element at index %d: %d\n", TEST_INDEX, removed);
    } else {
        printf("Failed to delete element at index %d\n", TEST_INDEX);
    }

    // Pop the last POP_COUNT elements
    printf("Popping last %d elements: ", POP_COUNT);
    for (int i = 0; i < POP_COUNT; i++) {
        int popped;
        if (int_vec_pop(int_vec, &popped) == 0) {
            printf("%d ", popped);
        }
    }
    printf("\n");
    ...
    int_vec_len(int_vec, &len);
    printf("Vector length after deletion and pops: %zu\n", len);
